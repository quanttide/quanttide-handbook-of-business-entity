...
- qtdata/connect/email.md：新增订单邮件工作流（承接→报价→合同→执行→验收→交付→发票→收尾全流程邮件模板）
- qtdata/index.md：目录新增「订单邮件工作流」入口
- myst.yml：量潮数据目录新增 connect/email.md
...
- connect/index.md：重写为简洁手册结构——一般方法（两问判断）→ 内部沟通（策略+模板+清单）→ 外部沟通（策略+模板+清单）；删除沟通渠道表、大客户补充、沟通前自检等冗余章节
